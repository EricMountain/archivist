...
**Still not verified live**: the approximate-date marker (every photo swiped through in this pass had real EXIF, so `takenAtSrc == "exif"` and the marker never had a reason to appear — this needs a photo actually lacking EXIF, which the synthetic load-test generator may not have produced any of) and pinch-zoom itself (two-finger gestures can't be synthesized through separate `adb shell input` invocations — each is its own process, so there's no way to drive two pointers as one continuous multitouch gesture without lower-level `sendevent` scripting, which this pass didn't attempt). Both are implemented and covered by the code as read, just not empirically confirmed the way the rest of this step now is. Status stays `partial` on the strength of those two specific gaps, not because anything found live is still broken. |
