...
**`gradle/gradle-daemon-jvm.properties` pinning `toolchainVendor=jetbrains` broke both
CI workflows outright.** That file constrains the JVM Gradle itself runs on (not the
Kotlin/Android compile toolchain), and it's honored even with `--no-daemon` — the error
says so explicitly ("a single-use Daemon process will be forked"). It was generated by
running `./gradlew updateDaemonJvm` on a machine where the active JDK was Android
Studio's bundled JetBrains Runtime, which the task captured as a hard vendor
requirement. `ubuntu-latest` runners have no JetBrains-vendor JDK, and Gradle's
`foojay-resolver-convention`-based auto-provisioning has no download source for that
vendor on Linux x86_64, so every job failed immediately with `Unable to download
toolchain matching the requirements (...vendor=JetBrains...): No defined toolchain
download url for LINUX on x86_64 architecture` — both `android-ci.yml` and
`android-release.yml` use this, so this silently broke `main` CI for several commits
before being caught by an actual release attempt. Fix: drop the `toolchainVendor` line
from the properties file, keeping only `toolchainVersion=21` — nothing here needs a
JetBrains-specific JDK, and Temurin (what `setup-java` installs in CI) satisfies the
version-only requirement. **`./gradlew updateDaemonJvm` itself is currently broken** on
this repo's Gradle/plugin combination (`foojay-resolver-convention` 0.10.0 against
Gradle 9.5): it fails with `Class org.gradle.jvm.toolchain.JvmVendorSpec does not have
member field 'IBM_SEMERU'` before it can even write the file — hand-edit
`gradle-daemon-jvm.properties` directly instead of trying to regenerate it until that's
resolved (bumping the foojay plugin may fix it; not yet verified).
