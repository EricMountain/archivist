...
| 2.5 Key enrolment and recovery code | partial | `KeyCustody.kt`/`DeviceKeystore.kt` (`:core:crypto`); `EnrolmentRepository.kt`/`EnrolmentViewModel.kt`/`EnrolmentScreen.kt`, `data/local/EnrolmentStore.kt`, `data/repo/MasterKeyHolder.kt` (`:app`); wired into `MainActivity` after sign-in and into `ArchivistApplication.onTrimMemory`. First-device enrolment, later-device recovery, silent unlock on restart, `KeyPermanentlyInvalidatedException` re-enrolment, `UserNotAuthenticatedException` handling, and (2026-08-30, see "Last audit") `NoSecureLockScreenException` handling are all implemented, covered by **94 passing JVM unit tests across both modules** (`:core:crypto` 10, `:app` 84). **2026-08-30, live against `dev`** (first time any part of 2.5 has run against a real deployed backend rather than a fake/local one; exact AVD/API not recorded): scenario 1 of the step's three "Done when" clauses confirmed clean (fresh enrol, restart, silent unlock). Scenario 2 (a second install recovering via code only) was attempted and crashed — root cause found and fixed this pass, see "Last audit"; not yet re-verified live now that the fix exists. Scenario 3 (lock-screen change → re-enrolment prompt) was exercised as "change PIN to a different PIN, then restart" and did *not* trigger `KeyPermanentlyInvalidatedException` — unclear whether that's correct Android behavior (the platform's documented invalidation trigger is disabling/resetting the secure lock screen, not necessarily changing to a new PIN while keeping one) or a real gap; recorded as an open question rather than guessed at, see "Last audit". Two real product-level gaps remain, written up as open questions 3 and 4 in `design.md`: (3) `PURPOSE_AGREE_KEY` needs API 31 but `minSdk` is 28, so there is no working hardware-backed device-wrap route at all below that; (4) no endpoint lets a later device fetch the owner's wrapped `hashSecret`, which plan step 2.10 will need. Still open from before: a real biometric (as opposed to PIN/pattern/password) satisfying the auth window, and the `NeedsDeviceUnlock` UI path's `KeyguardManager.createConfirmDeviceCredentialIntent` hand-off. Retrying `finishFirstEnrolment()` after a partial failure is still not idempotent — wasteful but not unsafe, not fixed since it's beyond this step's literal "Done when". Status stays `partial`: one of three required scenarios confirmed live, two remain (one blocked on a re-test, one on clarifying expected platform behavior). |
